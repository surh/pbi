...
#p1
ggsave("pi_experiment.png",p1)

# Plot overal distribution
p1 <- ggplot(Dat,aes(x = Pi_content,fill = Bacteria)) +
  geom_density(alpha = 0.3) +
  scale_x_log10(breaks = c(1, 5, 10, 15, 20)) +
  theme_classic()
#p1
ggsave("p1_distributions.png",p1)
...
for(syncom in communities){
  ...
  f1 <- log(Pi_content) ~ Bacteria + Experiment
...
write.table(Res,"pi_single_community_test.txt", sep = "\t", quote = FALSE, row.names = FALSE)
...
p1 <- ggplot(dat, aes(x = SynCom1, y = SynCom2)) +
  ...
  theme_classic()
p1
ggsave("pi_images/heatmap_mix_estimate.png",p1, width = 8, height = 7)
rm(singlecoms,dat)
...
p1 <- ggplot(Full, aes(x = SynCom1, y = SynCom2)) +
  ...
  theme_classic()
p1
ggsave("pi_images/heatmap_mix_full.png",p1, width = 8, height = 7)
...
p1 <- ggplot(dat,aes(x = Estimate, y = Predicted)) +
  ...
  theme_classic()
p1
ggsave("pi_images/estimate_vs_pred.png",p1, width = 7, height = 7)
